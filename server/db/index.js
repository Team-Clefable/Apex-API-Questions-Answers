...
module.exports = {
  ...
  queryAllQuestions: (productId, callback) => {
    let queryStr = `SELECT product_id, json_agg(json_build_object
     ...
     (SELECT coalesce(json_object_agg (answers.id, json_build_object ('id', id, 'body', body, 'date', date, 'answerer_name', answerer_name, 'helpfulness', helpfulness, 'photos',
     (SELECT coalesce(json_agg (json_build_object ('id', answers_id, 'url', url)
     ), '[]')
